    ...
    Public Class AquariumExtenderBase
        ...
        Public Shared Sub RegisterFrameworkSettings(ByVal p As Page)
            If Not (p.ClientScript.IsStartupScriptRegistered(GetType(AquariumExtenderBase), "TargetFramework")) Then
                ...
                p.ClientScript.RegisterStartupScript(GetType(AquariumExtenderBase), "TouchUI", String.Format("var __settings={{appInfo:""TestTraining|{0}"",mobileDisplayDensity:""Auto"",desktopDi"& _ 
                            "splayDensity:""Condensed"",mapApiIdentifier:"""",labelsInList:""DisplayedBelow"",label"& _ 
                            "sInForm:""AlignedLeft"",initialListMode:""SeeAll"",buttonShapes:true,sidebar:""Landsc"& _ 
                            "ape"",promoteActions:true,transitions:"""",theme:""Aquarium"",maxPivotRowCount: {1},h"& _ 
                            "elp:true,ui:""TouchUI""{2}}};", BusinessRules.JavaScriptString(HttpContext.Current.User.Identity.Name), ApplicationServices.Create().MaxPivotRowCount, siteContent), true)
            End If
        End Sub
